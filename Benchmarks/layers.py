...
for L in [2,4,6,8,10]:
    dum_input = np.expand_dims(np.ones([L*L]),0)
    graph = Square(L)
    ...
    runtime = timeit(lambda: apply_matrix(dum_input).block_until_ready(),number=10)/10
    print('compile time = ' + str(compile_time) + ' runtime is ' + str(runtime))
...
for L in [2,4,6,8,10]:
    ...
    ma_fft = DenseEquivariant(symmetries=product_table,mode='fft',shape=graph.extent, in_features=4,out_features=4)
    ma_irreps = DenseEquivariant(symmetries=irreps,mode='irreps',in_features=4,out_features=4)
    ma_matrix = DenseEquivariant(symmetries=product_table,mode='matrix',in_features=4,out_features=4)

    dum_input = np.zeros([1,4,len(sg)])
    ...
    runtime = timeit(lambda: apply_irreps(dum_input).block_until_ready(),number=10)/10
    print('compile time = ' + str(compile_time) + ' runtime is ' + str(runtime))
