...
{'a': 1, 'b': (1, 1+2.j, 3.j)},
{'a': jnp.ones(100), 'b': (jnp.ones((1,2))+3.j, jnp.ones(3),(jnp.ones((3,1))+3.j, jnp.ones((2,2))+3.j))}
])
...
def test_tree_to_real(tree):
    ...
    assert not jax.tree_util.tree_reduce(operator.or_, jax.tree_map(jnp.iscomplexobj, tree_real))
