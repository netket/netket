...
The constraints supported on the built-in hilbert spaces are:
 ...
 array([[0., 2.],
        ...
        [2., 0.]])
 ```
 - It is also possible to define a custom (Homogeneous) hilbert space with a custom constraint. For detailed information on implementing custom constraints, see {doc}`../advanced/custom_hilbert_constraints`. 
