...
* When performing algebric operations (*, -, +) on pairs of :class:`operator.LocalOperator`, the dtype of the result iscomputed using standard numpy promotion logic. 
...
* :class:`AbstractMachine` has been removed. It's functionality is now split among the model itself, which is defined by the user and :class:`variational.MCState` for pure states or :class:`variational.MCMixedState` for mixed states.
  ...
  * The model, in general is composed by two functions, or an object with two functions: an `init(rng, sample_val)` function, accepting a :ref:`jax.random.PRNGKey` object and an input, returning the parameters and the state of the model for that particular sample shape, and an `apply(params, samples, **kwargs)` function, evaluating the model for the given parameters and inputs.
