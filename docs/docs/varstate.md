...
- {class}`~netket.vqs.FullSummationState`, which is a version of {class}`~netket.vqs.MCState` without sampling, performing the full summation over the whole Hilbert space and computing expectation values and gradients exactly.
...
## Using FullSummationState for testing

In order to check the performance of a given model on small test systems, it is possible to use {class}`~netket.vqs.FullSummationState` which can be used in place of {class}`~netket.vqs.MCState` and computes quantities not by stochastic sampling but by evaluation of the ansatz over the full Hilbert space basis. (Therefore, it is infeasible to use for system sizes beyond ED.)
This can be helpful to diagnose potential errors arising from Monte Carlo sampling.

```python
> vs = nk.vqs.FullSummationState(hilbert, nk.models.RBM())
```
