...
## Advanced: MPI backend for CPU-only distributed computing

:::{warning}
**Experimental feature**: This MPI backend is experimental and **CPU-only** - it should not be used with GPUs. For GPU-based distributed computing, use the standard JAX distributed setup described above. Please let us know if this works for your use case.
:::
