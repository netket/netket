...
class Hypercube : public AbstractGraph {
 ...
 public:
  ...
  /// Given a site's index, returns sites' coordinates as a `Ndim()`-dimensional
  /// vector, returns the site's index.
  std::vector<int> Site2Coord(int site) const {
    if (site < 0 || site >= Nsites()) {
      throw InvalidInputError("Invalid site in Hypercube");
    }
