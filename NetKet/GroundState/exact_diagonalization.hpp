...
std::vector<double> eigenvalues_lanczos_run(const matrix_t& matrix,
  ...
  if (iter.error_code() == 1)
    WarningMessage() << "Warning: Lanczos algorithm did NOT converge in "
                     << iter.max_iterations() << " steps!\n";
...
std::vector<double> eigenvalues_lanczos(const Hamiltonian& hamiltonian,
  ...
  if (matrix_free) {  // Matrix-free computation
    ...
    eigs_lanczos = detail::eigenvalues_lanczos_run(matrix, random_gen, iter);
...
void write_ed_eigenvalues(const json& pars, const std::vector<double>& eigs) {
