...
@singledispatch
def _time_evo(self):
    raise ErrorException("unknown driver type {}".format(type(self)))


@_time_evo.register(Vmc)
def _time_evo_vmc(self):
    """
    Computes the derivative of the time-dependent ODE for
    a VMC (hamiltonian) driver.
...
def create_timevo(op, sampler, *args, julia=False, **kwargs):
    """
    Constructs the correct driver given the forward-time evolution
    generator `op`, the sampler `sampler` and other arguments.

        This function also constructs the function computing the function
        (or derivative) defining the ode, and returns it.

        If `julia=False`, then the function has signature fun(time, weights),
        otherwise it has signature fun(weights, params, time) and the second
        argument is ignored.
        This option is thought to be used with integrators from
        diffeq.py
    """
...
class TimeEvolution(AbstractVariationalDriver):
    ...
    def solver(self, METHOD, tspan=0.0, dt=None, adaptive=False, **kwargs):
