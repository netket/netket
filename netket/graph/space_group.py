...
from netket.utils.symmetry import (
    PGSymmetry,
    PointGroup,
    PermutationGroup,
    Permutation,
    ...
    * and the space group that is generated as their semidirect product.
    ...
    def point_group(self) -> PermutationGroup:
        ...
        for p in self.point_group_:
            ...
            perm = self.lattice.id_prom_position(p.preimage(self.lattice.positions))
    ...
    def _translations_along_axis(self, axis: int) -> PermutationGroup:
        ...
        if self.lattice._pbc[axis]:
            trans_list = [Identity()]
            # note that we need the preimages in the permutation
            trans_perm = self.lattice.id_prom_position(
    ...
    @struct.property_cached
    def translation_group(self) -> PermutationGroup:
        """
    ...
    def space_group(self) -> PermutationGroup:
        ...
        return self.translation_group @ self.point_group
    ...
    def _little_group_index(self, k: Array) -> Array:
        ...
        is_in_little_group = np.all(big_star == big.star[0], axis=1)
    ...
    def little_group(self, k: Array) -> PointGroup:
        ...
        """
        return PointGroup(self.point_group_[i] for i in self._little_group_index(k), dim = self.point_group_.ndim)
    ...
    def space_group_irreps(self, k: Array) -> Array:
        ...
        point_group_factors = self._little_group_irreps(k)[:, self.point_group_.conjugacy_table]
        # Wave vector factors
        big_star = np.tensordot(self.point_group_.to_array(), k, axes=1)
        big_star = self.lattice.to_reciprocal_lattice(big_star) * (2*pi / self.lattice.extent)
        trans_factors = []
        for axis in range(self.lattice.ndim):
            ...
            shape = [1] * axis + [n_trans] + [1] * (self.lattice.ndim - 1 - axis) + [len(self.point_group_)]
            trans_factors.append(factors.reshape(shape))
        trans_factors = reduce(np.multiply, trans_factors).reshape(-1, len(self.point_group_))

        # Multiply the factors together and sum over the "p" PGSymmetry axis
        # Translations are more major than point group operations
        return np.einsum('igp, tp -> itg', point_group_factors, trans_factors).reshape(point_group_factors.shape[0], -1)
