...
class Lattice(NetworkX):
    ...
    def space_group_builder(
        ...
        """
        Returns a `SpaceGroupBuilder` object that represents the space group
        generated by the translation symmetries of `self` and the elements of
        `point_group`. This object can be used to produce `PermutationGroup`s
        that describe the action of these groups on the sites of `self`, and to
        generate space group irreps for symmetrising wave functions.
    ...
    def point_group(self, point_group: Optional[PointGroup] = None) -> PermutationGroup:
        """
        Returns the action of `point_group` on the sites of `self` as a `PermutationGroup`.
        If no `point_group` is specified, uses the point group provided upon
        construction.
        """
    ...
    def translation_group(
        ...
        """
        return self.space_group_builder(None).translation_group(dim)
