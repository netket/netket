...
class Lattice(Graph):
    ...
    def __init__(
        ...
        *,
        ...
        site_offsets: _np.ndarray | None = None,
    ...
    def id_from_position(self, position: PositionT) -> int | Array:
        """
    ...
    def id_from_basis_coords(self, basis_coords: CoordT) -> int | Array:
        """
    ...
    @deprecated_new_name(
        "space_group",
        reason="Functionality of `SpaceGroupBuilder` and `SpaceGroup` merged",
    )
    def space_group_builder(
        ...
        """
        return self.space_group(point_group)

    @struct.property_cached
    def default_space_group(self) -> "SpaceGroup":
        ...
        if isinstance(self._point_group, PointGroup):
            ...
            raise RuntimeError(
                "Lattice has no default point group! Use space_group(point_group) instead."
            )
        return SpaceGroup(self, self._point_group)

    def space_group(self, point_group: PointGroup | None = None) -> "SpaceGroup":
        """
        Return the space group generated by the translation symmetries of `self`
        ...
        Returns:
            A `SpaceGroup` object that generates `PermutationGroup`s that encode
            the action of `point_group`, the translation group of `self`,
            and the space group obtained as their semidirect product as
            permutations of the sites of `self`.
            It also yields space group irreps for symmetrising wave functions.
        """
        from .space_group import SpaceGroup

        return (
            self.default_space_group
            if point_group is None
            else SpaceGroup(self, point_group)
        )

    def point_group(self, point_group: PointGroup | None = None) -> PermutationGroup:
        """
        Return the action of `point_group` on the sites of `self` as a
        `PermutationGroup`. If no `point_group` is specified, uses the point group
        provided upon construction.
        """
        return self.space_group(point_group).point_group

    def rotation_group(self, point_group: PointGroup | None = None) -> PermutationGroup:
        """
        Return the action of rotations (i.e. symmetries with determinant +1) in
    ...
    def space_group_representation(
        self, hilbert: DiscreteHilbert, point_group: PointGroup | None = None
    ):
        """
        Return the representation of the space group generated by the
        translation symmetries of `self` and the elements of `point_group`.
        The representation operators depends on the Hilbert space.
        ...
        """
        from netket.symmetry import Representation, construct_permutation_operator

        space_group = self.space_group(point_group)
        representation_dict = {
            perm: construct_permutation_operator(hilbert, perm) for perm in space_group
        }
        return Representation(space_group, representation_dict)
