...
class DenseEquivariant(Module):
    """An equivariant layer that takes a feature map with embeddings corresponding to various
    poses of a symmetry group, shape = [n_symm*in_features] and outputs a feature map of identical
    shape [n_symm*out_features]. This transformation respects the group structure such that
    g L({f}) = L(g{f}) where g is an element of the symmetry group and {f} are the feature embeddings.
    """

    group_algebra: Tuple
    """List of tuples generated by graph.group_algebra(automorphisms) that specifies the mapping
    between absolute and relative poses """
    in_features: int
...
def create_DenseEquivariant(
    symmetries: Union[AbstractGraph, Array],
    *args,
    **kwargs,
):
    """
    Constructor for DenseEquivariant Layer
    """
    if isinstance(symmetries, AbstractGraph):
        autom = np.asarray(symmetries.automorphisms())
        inv = inverse(autom)
    ...
    return DenseEquivariant(group_algebra=ga, *args, **kwargs)
