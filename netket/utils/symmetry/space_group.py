...
from .point_group import PointGroup, PGSymmetry
...
def space_group(lattice: Lattice, point_group: PointGroup):
    """Returns the space group generated by the translations of `lattice` and the
    point group `point_group` as a `PermutationGroup` acting on the sites of the
    same lattice."""
    return SpaceGroupBuilder(lattice, point_group).space_group
...
class SpaceGroupBuilder:
    ...
    * and the space group that is generated as their semidirect product.
        ...
        for i, p in enumerate(self.point_group_):
            if isinstance(p, Identity):
                ...
                perm = self.lattice.id_from_position(
                    p.preimage(self.lattice.positions), strict=True
                )
    ...
    def _translations_along_axis(self, axis: int) -> PermutationGroup:
        ...
        if self.lattice._pbc[axis]:
            ...
            trans_perm = self.lattice.id_from_position(
                self.lattice.positions - self.lattice.basis_vectors[axis], strict=True
            )
    ...
    def _little_group_index(self, k: Array) -> Array:
        ...
        big_star = (
            self.lattice.to_reciprocal_lattice(big_star, strict=True)
            % self.lattice.extent
        )
    ...
    def space_group_irreps(self, k: Array) -> Array:
        ...
        # Wave vector factors
        big_star = np.tensordot(self.point_group_.to_array(), k, axes=1)
        big_star = self.lattice.to_reciprocal_lattice(big_star, strict=True) * (
