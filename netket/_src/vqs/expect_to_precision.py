...
def _accumulate_stats(state, op_leaves, active, old_stats, *, max_lag):
    active = set(active)
    return [
        online_statistics(state.local_estimators(op).block_until_ready(), old_estimator=old, max_lag=max_lag)
        if i in active else old
...
def expect_to_precision(
    ...
    *,
    ...
    op_leaves, treedef = jax.tree.flatten(op, is_leaf=lambda x: isinstance(x, AbstractOperator))

    state.sample()
    stats_list = _accumulate_stats(state, op_leaves, range(len(op_leaves)), [None] * len(op_leaves), max_lag=max_lag)
    active = [i for i in range(len(op_leaves)) if _check_not_converged(stats_list[i], atol, rtol)]
    ...
    with tqdm(
        ...
        pbar.set_postfix(_format_postfix(stats_list[active[0] if active else 0], atol, rtol))
        try:
            while active and it < max_iter:
                state.sample(n_discard_per_chain=0)
                stats_list = _accumulate_stats(state, op_leaves, active, stats_list, max_lag=max_lag)
                active = [i for i in active if _check_not_converged(stats_list[i], atol, rtol)]
