...
class DeterminantVariationalState(VariationalState):
    ...
    Expectation values are computed deterministically using the one-body reduced
    density matrix, making this particularly efficient for mean-field calculations.

    Note:
        This state does not support Quantum Geometric Tensor (QGT) calculations
        directly. Use :meth:`to_fullsumstate` or :meth:`to_mcstate` to convert
        to a state type that supports QGT.
    """
    ...
    def quantum_geometric_tensor(self, qgt_T=None):
        ...
        and usable with optimization algorithms like SR.
        ...
        Note:
            The QGT is computed analytically and efficiently:
