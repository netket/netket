...
def get_sharding_spec(
    tree,
    *,
    axes: None | int | tuple[int, ...] | slice = None,
    ...
    """
    global_mesh = get_abstract_mesh()

    if isinstance(axes, int):
        axes = (axes,)
    if isinstance(axes, range):
        axes = tuple(axes)
    if only_if_explicit and not global_mesh.are_all_axes_explicit:
        return None
...
def drop_sharding_if_auto(sharding: NamedSharding | PartitionSpec | None, *, mesh=None):
    if sharding is None:
        return None
    ...
    # TODO: Support complicate cases where only some axes are auto
    if isinstance(sharding, NamedSharding):
        if len(sharding.mesh.auto_axes) > 0:
            return None
        else:
            return sharding
    elif isinstance(sharding, PartitionSpec):
        mesh = mesh or get_abstract_mesh()
        if len(mesh.auto_axes) > 0:
            return None
        else:
            return sharding
    else:
        raise TypeError(
            f"Unsupported sharding type: {type(sharding)}. Expected NamedSharding or PartitionSpec."
        )
...
def auto_axes_maybe(
    ...
    def wrapper(*args, **kwargs):
        ...
        if (sharding_mesh.empty and cur_mesh.empty) or cur_mesh.are_all_axes_auto:
