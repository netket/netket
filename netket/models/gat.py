...
class GATLayer(nn.Module):
    ...
    def __call__(self, x, A):
        ...
        ), "features must be perfectly divisible by heads"
        ...
        # Attention mechanism
        a_src = nn.Dense(
            1, dtype=self.dtype, kernel_init=self.kernel_init, use_bias=False
        )(h)
        a_dst = nn.Dense(
            1, dtype=self.dtype, kernel_init=self.kernel_init, use_bias=False
        )(h)
...
class GAT(nn.Module):
    ...
    def __call__(self, x):
        ...
        for out_features in self.layers:
            h = GATLayer(features=out_features, heads=self.heads, dtype=self.dtype)(
                h, self.A
            )
            h = self.activation(h)
