...
class GCNN(nn.Module):
    ...
    flattened_Cayley: Optional[HashableArray] = None
    ...
    def setup(self):
        ...
        if not np.any(self.flattened_Cayley) and not isinstance(
            self.symmetries, SymmGroup
        ):
            raise AttributeError(
                "Cayley Table must be specified if symmetries are given as an array"
            )

        if not np.any(self.flattened_Cayley):
            flattened_Cayley = self.symmetries.flattened_Cayley()
        else:
            flattened_Cayley = self.flattened_Cayley

        if not np.asarray(flattened_Cayley).shape[0] == np.square(self.n_symm):
            raise ValueError("Flattened Cayley table must have shape [n_symm*n_symm]")
        ...
        self.equivariant_layers = [
            nknn.DenseEquivariant(
                flattened_Cayley=flattened_Cayley,
