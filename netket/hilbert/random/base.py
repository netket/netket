...
def random_state(
    ...
    Args:
        ...
        key: The rng key generated by :func:`jax.random.key`.
        size: If provided, returns a batch of configurations of the form ``(size, N)`` if
            size is an integer or ``(*size, N)`` if it is a tuple and where ``N`` is the Hilbert
            space size. By default, a single random configuration with shape ``(N,)`` is
            returned.
...
def flip_state_batch(hilb, key, states, indxs):
    ...
    if is_sharded(states):
        ...
        # Use shard map only if states have explicit sharding
        if not len(mesh.auto_axes + mesh.manual_axes) > 0:
            ...
            _flip_state_scalar_vmap = jax.shard_map(
