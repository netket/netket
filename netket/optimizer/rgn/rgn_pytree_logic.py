...
def rhessian_real_holo(
    ...
    def _rhessian_real_holo(logpsi, params, σ, σp, mels):
        ...
        scale = mels*jnp.exp(logpsi(params,σp) - single_sample(logpsi)(params,σ))
        res, _ = vjp_fun(scale)
        return res
...
def mat_vec(v: PyTree, oks: PyTree, rhes: PyTree, mean_grad: PyTree, eps: float, en: float, diag_shift: float) -> PyTree:
    ...
    res2 = tree_conj(_vjp(v, _jvp(mean_grad, v).conjugate()))
    res3 = tree_conj(_vjp(oks, _jvp(oks, v).conjugate()))

    res = jax.tree_multimap(lambda w,x,y,z: x - y - (en - eps)*z + eps*diag_shift*w,v,res,res2,res3)
