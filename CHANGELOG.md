...
* The multi-GPU solvers {func}`netket.optimizer.solver.cholesky_distributed` and {func}`netket.optimizer.solver.pinv_smooth_distributed` now target [JAXMg](https://flatironinstitute.github.io/jaxmg/) 1.1.1 or later (`pip install 'netket[jaxmg]'`), which is built on NVIDIA's cuSOLVERMp instead of the deprecated cuSOLVERMg. This means they now also work **across multiple nodes**, but they require **one python process per GPU** (run your script with `djaxrun`, or call {func}`jax.distributed.initialize` yourself). Older `jaxmg` releases are no longer supported, and raise an informative error [issue #2258](https://github.com/netket/netket/issues/2258), [PR #2265](https://github.com/netket/netket/pull/2265).
  ...
  `jaxmg` is no longer part of the `extra` extra, because every `jaxmg` release pins an exact `jax` version (e.g. `jaxmg` 1.3.0 requires `jax==0.11.1`).
...
* `driver.run(n_iter)` now stops `n_iter` steps after the step it was called at, even if a callback changes the step count in `on_run_start` (for example when resuming from a checkpoint), instead of running `n_iter` more steps from there [PR #2284](https://github.com/netket/netket/pull/2284).
* Drivers can now be restored from a checkpoint into a newly created driver. Before, values recomputed at every step were saved too, and some buffers were only created at the first step, so a new driver did not match a saved one: {class}`netket.driver.VMC_SR` failed to continue after a restore, and restoring with orbax into a template failed for all drivers. Values recomputed at every step are no longer saved, the SPRING momentum buffer and the MCMC diagnostics of {class}`~netket.driver.VMC_SR` are created with the driver, and parameters split into real and imaginary parts can be serialized. Files written by older versions still load: fields that are no longer saved are ignored [PR #2283](https://github.com/netket/netket/pull/2283).
* {class}`netket.callbacks.EarlyStopping` now saves its best value and patience counter, so a run resumed from a checkpoint keeps counting instead of starting over. A job restarted more often than every `patience` steps could otherwise never stop early. Files saved by older versions still load [PR #2285](https://github.com/netket/netket/pull/2285).
* {class}`netket.callbacks.ConvergenceStopping` and {class}`netket.callbacks.InvalidLossStopping` now also save their state, so a run resumed from a checkpoint behaves as if it had not been interrupted. Before, `InvalidLossStopping` stopped at the first invalid step after a resume, ignoring `patience` [PR #2287](https://github.com/netket/netket/pull/2287).
...
* {class}`netket.logging.MLFlowLog` no longer deadlocks multi-process (sharded) runs: it returned early on non-master ranks *before* walking the log tree, so a host-side collective triggered during the walk (reducing a sharded {class}`netket.stats.Stats`, materialising a sharded array) ran on rank 0 alone and hung every other rank. The tree is now walked on all ranks and only the write is gated to the master, mirroring {class}`netket.logging.HDF5Log` [PR #2274](https://github.com/netket/netket/pull/2274).
...
### Bug Fixes
* Fixed a bug preventing NetKet from working with Python 3.14 [Issue #2223](https://github.com/netket/netket/issues/2223).
...
* {func}`netket.jax.tree_ravel` is deprecated in favor of {func}`jax.flatten_util.ravel_pytree`. Use `from jax.flatten_util import ravel_pytree` instead of `nk.jax.tree_ravel` [#2164](https://github.com/netket/netket/pull/2164).
