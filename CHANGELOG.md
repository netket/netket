...
### Bug Fixes
* The multi-GPU solvers {func}`netket.optimizer.solver.cholesky_distributed` and {func}`netket.optimizer.solver.pinv_smooth_distributed` now require `jaxmg>=0.0.9,<1.0.0`, and raise an informative error instead of failing obscurely if an unsupported version is installed. `jaxmg` 1.0 rewrote its API on top of NVIDIA's cuSOLVERMp and requires one python process per GPU, which NetKet does not support yet [issue #2258](https://github.com/netket/netket/issues/2258).
