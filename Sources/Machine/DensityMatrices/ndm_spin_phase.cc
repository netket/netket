...
NdmSpinPhase::NdmSpinPhase(std::shared_ptr<const AbstractHilbert> hilbert,
                           int nhidden, int nancilla, int alpha, int beta,
                           bool useb, bool useh, bool used)
...
void NdmSpinPhase::InitRandomPars(int seed, double sigma) {
...
void NdmSpinPhase::InitLookup(VisibleConstType v_r, VisibleConstType v_c,
                              LookupType &lt) {
  if (lt.VectorSize() == 0) {
  ...
  lt.V(0) = (W1_.transpose() * v_r + h1_);
  lt.V(1) = (W2_.transpose() * v_r + h2_);
  lt.V(2) = (W1_.transpose() * v_c + h1_);
  lt.V(3) = (W2_.transpose() * v_c + h2_);

  lt.V(4) = (0.5 * U1_.transpose() * (v_r + v_c) + d1_);
  lt.V(5) = (0.5 * U2_.transpose() * (v_r - v_c));
}

void NdmSpinPhase::UpdateLookup(VisibleConstType v_r, VisibleConstType v_c,
                                ...
                                LookupType &lt) {
  for (std::size_t s = 0; s < tochange_r.size(); s++) {
    const int sf = tochange_r[s];
    lt.V(0) += W1_.row(sf) * (newconf_r[s] - v_r(sf));
    lt.V(1) += W2_.row(sf) * (newconf_r[s] - v_r(sf));

    lt.V(4) += 0.5 * U1_.row(sf) * (newconf_r[s] - v_r(sf));
    lt.V(5) += 0.5 * U2_.row(sf) * (newconf_r[s] - v_r(sf));
  }

  for (std::size_t s = 0; s < tochange_c.size(); s++) {
    const int sf = tochange_c[s];
    lt.V(2) += W1_.row(sf) * (newconf_c[s] - v_c(sf));
    lt.V(3) += W2_.row(sf) * (newconf_c[s] - v_c(sf));

    lt.V(4) += 0.5 * U1_.row(sf) * (newconf_c[s] - v_c(sf));
    lt.V(5) -= 0.5 * U2_.row(sf) * (newconf_c[s] - v_c(sf));
  }

}  // namespace netket

VectorType NdmSpinPhase::DerLog(VisibleConstType v_r, VisibleConstType v_c) {
  LookupType ltnew;
  InitLookup(v_r, v_c, ltnew);
  return DerLog(v_r, v_c, ltnew);
}
...
VectorType NdmSpinPhase::DerLog(VisibleConstType v_r, VisibleConstType v_c,
  ...
  if (useb_) {
    der.head(nv_) = 0.5 * (v_r + v_c);
    der.segment(impar, nv_) = I_ * 0.5 * (v_r - v_c);
  }
  ...
  thetas_a1_ = 0.5 * U1_.transpose() * (v_r + v_c) + d1_;
  thetas_a2_ = 0.5 * U2_.transpose() * (v_r - v_c);
  ...
  MatrixType wder =
      0.5 * (v_r * lnthetas_r1_.transpose() + v_c * lnthetas_c1_.transpose());
  der.segment(initw_1, nv_ * nh_) =
      Eigen::Map<VectorType>(wder.data(), nv_ * nh_);

  wder = 0.5 * I_ *
         (v_r * lnthetas_r2_.transpose() - v_c * lnthetas_c2_.transpose());
  ...
  MatrixType uder = 0.5 * (v_r + v_c) * lnpi_.transpose();
  der.segment(initu_1, nv_ * na_) =
      Eigen::Map<VectorType>(uder.data(), nv_ * na_);

  uder = 0.5 * I_ * (v_r - v_c) * lnpi_.transpose();
...
Complex NdmSpinPhase::LogVal(VisibleConstType v_r, VisibleConstType v_c) {
  RbmSpin::lncosh(W1_.transpose() * v_r + h1_, lnthetas_r1_);
  RbmSpin::lncosh(W2_.transpose() * v_r + h2_, lnthetas_r2_);
  RbmSpin::lncosh(W1_.transpose() * v_c + h1_, lnthetas_c1_);
  RbmSpin::lncosh(W2_.transpose() * v_c + h2_, lnthetas_c2_);
  ...
  auto gamma_1 =
      0.5 * (lnthetas_r1_.sum() + lnthetas_c1_.sum() + (v_r + v_c).dot(b1_));

  auto gamma_2 =
      0.5 * (lnthetas_r2_.sum() - lnthetas_c2_.sum() + (v_r - v_c).dot(b2_));
...
// are being flipped
VectorType NdmSpinPhase::LogValDiff(
  ...
  assert(tochange_r.size() == tochange_c.size());

  thetas_r1_ = (W1_.transpose() * v_r + h1_);
  thetas_r2_ = (W2_.transpose() * v_r + h2_);
  thetas_c1_ = (W1_.transpose() * v_c + h1_);
  thetas_c2_ = (W2_.transpose() * v_c + h2_);
  ...
  const std::size_t nconn = tochange_r.size();

  VectorType logvaldiffs = VectorType::Zero(nconn);
  for (std::size_t k = 0; k < nconn; k++) {
    if (tochange_r[k].size() != 0 || tochange_c[k].size() != 0) {
      ...
      for (std::size_t s = 0; s < tochange_r[k].size(); s++) {
        const int sf = tochange_r[k][s];

        logvaldiffs(k) += 0.5 * b1_(sf) * (newconf_r[k][s] - v_r(sf));
        logvaldiffs(k) += 0.5 * I_ * b2_(sf) * (newconf_r[k][s] - v_r(sf));

        thetasnew_r1_ += W1_.row(sf) * (newconf_r[k][s] - v_r(sf));
        thetasnew_r2_ += W2_.row(sf) * (newconf_r[k][s] - v_r(sf));
        thetasnew_a1_ += 0.5 * U1_.row(sf) * (newconf_r[k][s] - v_r(sf));
        thetasnew_a2_ += 0.5 * U2_.row(sf) * (newconf_r[k][s] - v_r(sf));
      }
      for (std::size_t s = 0; s < tochange_c[k].size(); s++) {
        const int sf = tochange_c[k][s];

        logvaldiffs(k) += 0.5 * b1_(sf) * (newconf_c[k][s] - v_c(sf));
        logvaldiffs(k) -= 0.5 * I_ * b2_(sf) * (newconf_c[k][s] - v_c(sf));

        thetasnew_c1_ += W1_.row(sf) * (newconf_c[k][s] - v_c(sf));
        thetasnew_c2_ += W2_.row(sf) * (newconf_c[k][s] - v_c(sf));
        thetasnew_a1_ += 0.5 * U1_.row(sf) * (newconf_c[k][s] - v_c(sf));
        thetasnew_a2_ -= 0.5 * U2_.row(sf) * (newconf_c[k][s] - v_c(sf));
      }
...
Complex NdmSpinPhase::LogValDiff(VisibleConstType v_r, VisibleConstType v_c,
                                 const std::vector<int> &tochange_r,
                                 const std::vector<int> &tochange_c,
                                 const std::vector<double> &newconf_r,
                                 const std::vector<double> &newconf_c,
  ...
  if (tochange_r.size() != 0 || tochange_c.size() != 0) {
    RbmSpin::lncosh(lt.V(0).real(), lnthetas_r1_);
    RbmSpin::lncosh(lt.V(1).real(), lnthetas_r2_);
    RbmSpin::lncosh(lt.V(2).real(), lnthetas_c1_);
    RbmSpin::lncosh(lt.V(3).real(), lnthetas_c2_);
    ...
    for (std::size_t s = 0; s < tochange_r.size(); s++) {
      ...
      logvaldiff += 0.5 * b1_(sf) * (newconf_r[s] - v_r(sf));
      logvaldiff += 0.5 * I_ * b2_(sf) * (newconf_r[s] - v_r(sf));

      thetasnew_r1_ += W1_.row(sf) * (newconf_r[s] - v_r(sf));
      thetasnew_r2_ += W2_.row(sf) * (newconf_r[s] - v_r(sf));
      thetasnew_a1_ += 0.5 * U1_.row(sf) * (newconf_r[s] - v_r(sf));
      thetasnew_a2_ += 0.5 * U2_.row(sf) * (newconf_r[s] - v_r(sf));
    }
    for (std::size_t s = 0; s < tochange_c.size(); s++) {
      const int sf = tochange_c[s];

      logvaldiff += 0.5 * b1_(sf) * (newconf_c[s] - v_c(sf));
      logvaldiff -= 0.5 * I_ * b2_(sf) * (newconf_c[s] - v_c(sf));

      thetasnew_c1_ += W1_.row(sf) * (newconf_c[s] - v_c(sf));
      thetasnew_c2_ += W2_.row(sf) * (newconf_c[s] - v_c(sf));
      thetasnew_a1_ += 0.5 * U1_.row(sf) * (newconf_c[s] - v_c(sf));
      thetasnew_a2_ -= 0.5 * U2_.row(sf) * (newconf_c[s] - v_c(sf));
    }
